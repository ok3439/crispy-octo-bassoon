	...
	try:
		while True:
			try:
				...
				connection = http.client.HTTPConnection( HTTP_SERVER , HTTP_SERVER_PORT, timeout=2)
				#dataparam = "blah"
				connection.request( "GET", "/data=%s" % dataparam)
				#connection.request( "GET", "/")
				response = connection.getresponse()
				if response.status == 200:
					text = response.read()
					if len(text) > 0:
						logging.info("Thread http_client_function: relaying %d bytes (%s)" % (len(text),text))
						#with socket_lock:
						sock.sendall( text)
				else:
				...
				logging.info("Thread http_client_function: received http.client.NotConnected")
				#with socket_lock:
